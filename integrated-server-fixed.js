// INTEGRATED SERVER - Fixed Code Generation
...
Bun.serve({
    ...
    async fetch(req) {
        ...
        // API: Create new access code (admin only) - FIXED!
        if (path === '/api/codes' && method === 'POST') {
            ...
            try {
                ...
                console.log('Creating code with:', body);

                const type = body.type || 'season';
        ...
        // API: Get available games for code assignment
        if (path === '/api/games' && method === 'GET') {
            try {
                const games = db.prepare('SELECT id, name, date FROM games ORDER BY date DESC').all();
        ...
        try {
            ...
            if (filePath === '/') {
                filePath = '/integrated-dashboard.html';
            }
...
// Show existing codes on startup
...
// Show existing games
console.log('\n🎮 Existing Games:');
const existingGames = db.prepare('SELECT id, name FROM games').all();
...
Dashboard: http://localhost:${port}/integrated-dashboard.html

Default Logins:
- Admin: admin / changeme123
- Viewer Codes: See list above
